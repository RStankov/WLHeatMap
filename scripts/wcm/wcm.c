...
void printAvailableNetwork (PWLAN_AVAILABLE_NETWORK pNetwork)
{
	...
	printf("\tSignal quality: %lu%%", pNetwork->wlanSignalQuality);

	/* Default auth algorithm */
	printf("\tDefault authentication algorithm: %s", GetAuthAlgoString(pNetwork->dot11DefaultAuthAlgorithm));

	/* Default cipher algorithm */
	printf("\tDefault cipher algorithm: %s", GetCipherAlgoString(pNetwork->dot11DefaultCipherAlgorithm));
}
...
void printBssEntry (PWLAN_BSS_ENTRY pBss)
{
	...
	if (pBss != NULL)
	{
		...
		for (i = 0; i < 6; i++) {
			...
			printf("%s", itoa(pBss->dot11Bssid[i], buf, 16));
			if(i<5) printf(":"); //changing into the normal MAC format
		}
		printf("\n");
	}
}
